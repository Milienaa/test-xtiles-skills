...
2. **Planner destination — this skill has no way to update an existing tile's
   content, so every run creates a fresh set of tiles.** If the target page
   already has sections with the same headings — from a saved template or from
   an earlier run today — still write all the approved sections: do not read
   the page to check for matching headings first, and do not skip any section
   because a same-titled tile might already exist. One call to
   `xtiles_create_tiles_from_markdown_in_my_planner` with `period: "day"`
   (Daily) or `period: "week"` (Weekly), the resolved date, and all approved
   sections in a single markdown string. In `Both` mode, write the Daily
   sections to the day page and the Weekly sections to the week page — two
   calls, one per period, never mixed onto one page.
