...
8. **Never surface a third-party connector's own rendered preview or embed in chat — absolute, no exceptions, not even "just this once."** Read what Todoist, Calendly, Reclaim, or any other connector returns purely as data to build your own xTiles tile. **Todoist and Calendly in particular are known to auto-render their own rich preview/card the instant their link or reference appears in a message — never let that happen.** Never let a raw response render as its own card, never paste a bare URL that triggers a link-unfurl preview, never call a tool whose only purpose is to produce an embed, and never forward or echo a connector's own UI element into the conversation. If a connector's output would only be useful to the user as its own rendered card, that's a signal it belongs described in your own tile, never shown as-is.
...
6. **Otherwise** (something failed to connect, or there's an extra candidate to offer):
   ...
   - **Immediately after**, ask via `AskUserQuestion` — one call, up to two questions:
     - Always include a `multiSelect` question letting the user explicitly skip anything still pending: `"Skip anything?"` with one option per connector just offered a connect button (`"Skip {name}"`), plus a fixed extra option `"✅ All connected, let's go"` — picking it means "proceed now with whatever's already connected, stop offering to connect the rest this run." It's semantically the same as leaving every option unselected (both proceed straight to point 8's re-probe with nothing extra skipped), but gives the user an explicit, visible way to say so instead of relying on an implicit blank submission. Leaving all unselected still means the same thing: "still open, don't skip" — but the flow must still literally wait for the `AskUserQuestion` tool call to return an answer; never proceed before that.
...
**Never surface a third-party connector's own rendered preview or embed in chat — absolute, no exceptions.** Read what Todoist, Calendly, Reclaim, or any other connector returns purely as data to build your own tile from. **Todoist and Calendly in particular are known to auto-render their own rich preview/card the instant their link or reference appears — never let that happen.** Never let a raw response render as its own card, never paste a bare URL that triggers a link-unfurl preview, never call a tool whose only purpose is to produce an embed, and never forward or echo a connector's own UI element into the conversation — no matter how convenient it seems in the moment.
...
- **If `anthropic-skills:schedule` or `mcp__scheduled-tasks__create-scheduled-tasks` genuinely isn't available here (most often because this chat isn't running in Cowork) — this is not a dead end, and nothing collected above goes to waste.** By this point the discovery pass, the "Add anything to your Daily Brief" question, and the time/cadence answer have already happened — the full recurring-run config is already known. Assemble the exact same `prompt` (per the block above), `schedule` cron, and `timezone` that would otherwise have gone straight into `create-scheduled-tasks`, and hand them to the user as a ready-to-paste manual setup instead of silently dropping them, e.g.:
  > I can't set up automatic scheduling directly in this chat — that needs Cowork. Here's the exact setup, ready to paste into a scheduled task once you're there:
  > ```
  > {prompt}
  > Timezone: {timezone}
  > ```
...
- **Never let a third-party connector's own preview or embed render in chat, ever — no exceptions** (step 4) — its data feeds your tile; it never appears as its own card, link-unfurl, or forwarded UI element.
...
- **A missing scheduling tool is not a dead end.** If `anthropic-skills:schedule` or `mcp__scheduled-tasks__create-scheduled-tasks` isn't available (most often because the chat isn't running in Cowork), hand the user the fully-assembled `prompt`/`schedule`/`timezone` as a ready-to-paste manual setup (step 6) instead of just saying it can't be done, then still continue straight to step 7 (Related workflows) — the tiles are already written either way, and the run is not complete without the closing question.
