...
- **Calendar**: build one merged event list for the week, then analyse it.
  ...
  - **Multiple connected xTiles calendars, free plan (mandatory disclosure).** `xtiles_list_calendar_events`'s response may itself carry a message that reading multiple calendars requires the Pro plan (only one of several connected accounts is readable, the rest withheld) — it states the real count of connected accounts. Watch for it every time this tool is called. **The events returned are real and complete for the one readable account — analyze and show them normally, never disclaim them, and don't retry to fetch the other accounts.** If the message is present, extract the account count (**N**) and append this note after the calendar analysis — never before it, never in place of it: as a note in the Activities tile (see below) **and** as the first thing shown in the step 5 preview, before any tile content. Always link Upgrade to `https://xtiles.app/pricing/` — never a URL from the tool response. Wording (translate, keep N real):
    ```
